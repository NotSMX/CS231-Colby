...
public class Maze implements Iterable<Cell> {
    ...
    /**
     * EXTENSION: The density of this Maze. Each Cell independently has probability
     * {@code density} of being an MUD AND ICE.
     */
    private double mudProbability;
    ...
    public Maze(int rows, int columns, double density) {
        ...
        this.mudProbability = 0;
        this.iceProbability = 0;
        landscape = new Cell[rows][columns];
        reinitialize();
    }

    public Maze(int rows, int columns, double density, double mudProbability, double iceProbability) {
        this.rows = rows;
        this.cols = columns;
        this.density = density;
        this.mudProbability = mudProbability;
    ...
    /**
     * Initializes every Cell in the Maze. EXTENSION ONLY
     */
    public void reinitialize2() {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                ...
                } else if (randomValue < density + mudProbability) {
                    landscape[r][c] = new Cell(r, c, CellType.MUD);
                } else if (randomValue < density + mudProbability + iceProbability) {
