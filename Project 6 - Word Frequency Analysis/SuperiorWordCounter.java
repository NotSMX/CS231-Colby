 ...
 * Author: Daniel Yu
 * 
 * An alternate version of WordCounter that ignores cases, and special characters when evaluating words.
 */

import java.util.ArrayList;
import java.io.*;

public class SuperiorWordCounter {

    private MapSet<String, Integer> wordCounts;
    private int wordCount;

    // constructor, where data_structure is either "bst" or "hashmap"
    public SuperiorWordCounter(String data_structure) {
    ...
    public ArrayList<String> readWords(String filename) {
        ...
        try {
            ...
            while (line != null) {
                // Remove special characters and split words by non-alphabetic characters
                String[] lineWords = line.replaceAll("[^a-zA-Z ]", "").toLowerCase().split("[\\s]+");

                for (String word : lineWords) {
                    // Ignore empty strings (resulting from multiple spaces) and add lowercase words
                    // to the list
                    if (!word.isEmpty()) {
                        words.add(word);
                    }
    ...
    public static void main(String[] args) {
        for (int year = 2008; year < 2009; year++) {
            ...
            // String filename = "reddit_comments_" + year + ".txt";
            String filename = "New Text Document.txt";

            // Use the BST
            SuperiorWordCounter wc = new SuperiorWordCounter("BST");
            ...
            wc.writeWordCount("betterOutput" + year + ".txt");
        }
    }
